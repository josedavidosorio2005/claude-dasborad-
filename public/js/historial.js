...
function renderHist(filter,tipo){
  ...
  tbody.innerHTML=list.map(function(h){
    return '<tr>'+
      '<td style="white-space:nowrap;font-size:0.79rem;color:#5b7a8a">'+h.fecha+'</td>'+
      '<td><strong style="color:#0d4a5e;font-size:0.84rem">'+h.nombre+'</strong><br>'+
        '<span style="font-size:0.74rem;color:#7a9ba8;font-family:monospace">@'+h.username+'</span></td>'+
      '<td style="font-size:0.78rem;color:#2a4a58">'+h.rol+'</td>'+
      '<td><span class="hist-badge hb-'+h.accion+'">'+h.accion+'</span></td>'+
      '<td style="font-size:0.82rem;color:#2a4a58">'+h.actor+'</td>'+
      '<td style="font-size:0.80rem;color:#5b7a8a;max-width:220px">'+h.detalle+'</td>'+
      '</tr>';
  }).join('');
}
function filterHist(){
